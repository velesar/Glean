...
@router.get("/weekly")
async def get_weekly_report(weeks: int = Query(1, ge=1, le=12)):
...
async def get_weekly_report_raw(weeks: int = Query(1, ge=1, le=12)):
...
async def get_changelog_report(days: int = Query(7, ge=1, le=90)):
...
async def get_changelog_report_raw(days: int = Query(7, ge=1, le=90)):
...
@router.get("/index")
async def get_tools_index_report():
